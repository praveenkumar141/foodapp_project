...
export const loadUser = (accessToken) => async (dispatch) => {
  try {
    ...
    const { id, email, username, password, address, roles, cart } = res.data;
    var isAdmin = false;
    roles.forEach((role) => {
    ...
    dispatch({
      type: USER_LOADED,
      payload: { id, email, username, password, address, roles, cart, isAdmin },
    });
...
export const deleteAccount = (id) => async (dispatch) => {
  try {
    ...
    dispatch(
      setAlert(errors.message + ": " + "Cannot delete account", "danger")
    );
  }
};
